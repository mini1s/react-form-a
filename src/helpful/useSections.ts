...
type UseSectionsReturnValue = {
  ...
  setCompleteSections: React.Dispatch<
...
export const useSections: UseSections = ({ docRef, user, path }) => {
  ...
  useEffect(() => {
    (async () => {
      ...
      try {
        ...
        setCompleteSections(
          (path ? data?.[path]?.completeSections : data?.completeSections) ||
            {},
        );
        setSubmitted(
          (path ? data?.[path]?.submitted : data?.submitted) || false,
        );
        setLoading(false);
      } catch (error) {
        console.error(error);
  ...
  const submit = async () => {
    ...
    try {
      await setDoc(
        docRef,
        { [path ? `${path}.submitted` : "submitted"]: true },
        { merge: true },
      );
